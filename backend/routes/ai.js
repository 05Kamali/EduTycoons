...
router.post('/chatbot', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = req.user.role === 'jobseeker' 
    ...
    if (process.env.OPENAI_API_KEY) {
      ...
      res.json({
        response: "AI Chatbot is not configured. Please add OPENAI_API_KEY to your environment variables. For now, I can provide general guidance: continue learning, network actively, and tailor your applications to each job posting."
      });
    }
  } catch (error) {
...
router.get('/learning-resources', auth, async (req, res) => {
  try {
    ...
    if (type === 'all' || type === 'github') {
      try {
        const githubResponse = await axios.get(
          `https://api.github.com/search/repositories?q=${encodeURIComponent(skill)}+language&sort=stars&order=desc&per_page=5`
        );
    ...
    res.status(500).json({ message: 'Failed to fetch learning resources', error: error.message });
  }
});
